...
1. **תיקון תפריט עליון במובייל** - מילה אחת תופסת 2 שורות, צריך לתקן
2. **תיקון כפתורים בפרופיל מקוצר** - בחלון החיפוש, הכפתורים עולים אחד על השני
3. **תיקון צבע כפתור פתיחת צ'אט** - הצבע ב-ON CLICK לא טוב, לא רואים כלום

### בעדיפות בינונית:
4. **הוספת שדה מצב דת** - להוסיף פרמטר מצב דת לפרופיל
   - סדר מומלץ (מחילוני לחרדי):
     - חילוני
     - מסורתי
     - דתי לאומי
     - דתי
     - חרדי
     - חוזר בתשובה
   - **הערה**: המשתמש ישלח את הרשימה המלאה בהמשך
5. **הוספת דף ניהול למנהל** - דף ניהול עבור מי שהוגדר כמנהל
   - כרגע רק עבור המשתמש שהוא שורש העץ
   - יהיה נסתר לכל שאר המשתתפים
6. **הוספת אופציה ל-"אנלייק"** (חסימת משתמש)
7. **הוספת סוגי משתמשים** (מנהל, משדך, מחפש)
8. **שיפור עיצוב - אפשרות חיפוש בטלפון**
